...
const Scanner: React.FC<ScannerProps> = ({
  ...
  const generateValidBarcode = (): string => {
    const validPrefixes = ['73', '62', '01', '20', '80'];
  ...
  const startBarcodeScanning = () => {
    ...
    barcodeScanIntervalRef.current = setInterval(() => {
      performScan();
  ...
  const performScan = () => {
    ...
    const simulateDetection = Math.random() > 0.7; // 30% chance to detect
    
    if (simulateDetection) {
      // Generate random box within the central area of the canvas
